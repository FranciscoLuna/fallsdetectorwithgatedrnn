...
class FallDatabase():
    ...
    #
    def structuredData(self, dataValues, dataClasif, nTestSubjects = 4, softData=False, usersForValidation = None):
        ...
        if not(usersForValidation):
            ...
            subjectsType1SelectedForTest = usersForValidation[0]
            subjectsType2SelectedForTest = usersForValidation[1]
        ...
        for subject in dataValues.keys():
            # print(subject)
            # if subject in ["SA15", "SA03", "SE06", "SA22", "SA19", "SE14", "SE08", "SE07"]:
            #     print(subject)
            #     continue
...
def divideDataSetForTrain(dataValues, dataLabels, nTestUsers=4, usersForValidation = None):
    ...
    trainData, trainClasif, testData, testClasif, dataOrganization = \
    sisFallDatabaseInstance.structuredData(dataValues, dataLabels, nTestUsers, False, usersForValidation)
    
    return trainData, trainClasif, testData, testClasif, dataOrganization

def prepareDataSetForTrain(dataFilesRootpath='./SisFallData/', nTestUsers=4, usersForValidation = None):
    ...
    if os.path.isfile(dataFilesRootpath + dataForTrainCompleteName):
        ...
        dataValues, dataLabels = prepareDataset('../')
        trainData, trainClasif, testData, testClasif, dataOrganization = divideDataSetForTrain(dataValues, dataLabels, nTestUsers, usersForValidation)
        del dataValues
        del dataLabels
...
def loadDataSetInBlocks(dataFilesRootpath='./SisFallData/', nTestUsers=4, windowSize=128, stride=64, randomOrder = False, usersForValidation = None):
    
    print("Loading data...")
    
    trainData, trainClasif, testData, testClasif, dataOrganization = prepareDataSetForTrain(dataFilesRootpath, nTestUsers, usersForValidation)
